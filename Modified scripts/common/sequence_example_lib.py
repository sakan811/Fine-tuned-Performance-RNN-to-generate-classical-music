...
def get_padded_batch(file_list, batch_size, input_size, label_shape=None,
  ...
  """
  # Determine the compression type based on the filename extension.
  compression_type = 'GZIP' if file_list[0].endswith('.gz') else None
  
  file_queue = tf.train.string_input_producer(file_list)
  
  # Instantiate the TFRecordReader with the determined compression type.
  reader_options = tf.io.TFRecordOptions(compression_type=compression_type)
...
def count_records(file_list, stop_at=None):
  ...
  for tfrecord_file in file_list:
      tf.logging.info('Counting records in %s.', tfrecord_file)
      compression_type = 'GZIP' if tfrecord_file.endswith('.tfrecord.gz') else None
      for _ in tf.io.tf_record_iterator(tfrecord_file, options=tf.io.TFRecordOptions(compression_type=compression_type)):
          num_records += 1
          if stop_at and num_records >= stop_at:
              tf.logging.info('Number of records is at least %d.', num_records)
              return num_records
