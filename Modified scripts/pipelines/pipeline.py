...
def tf_record_iterator(tfrecord_file, proto):
    ...
    """
    options = None
    if tfrecord_file.endswith('.gz'):
        options = tf.io.TFRecordOptions(compression_type='GZIP')
    
    for raw_bytes in tf.io.tf_record_iterator(tfrecord_file, options=options):
        yield proto.FromString(raw_bytes)
...
def run_pipeline_serial(pipeline,
                        input_iterator,
                        output_dir,
                        output_file_base=None, use_compression=False):
  ...
  if output_file_base is None:
    if use_compression:
        output_paths = [os.path.join(output_dir, name + '.tfrecord.gz')
                        for name in output_names]
    else:
        output_paths = [os.path.join(output_dir, name + '.tfrecord')
                        for name in output_names]
      ...
      if use_compression:
          output_paths = [os.path.join(output_dir,
                                      '%s_%s.tfrecord.gz' % (output_file_base, name))
                          for name in output_names]
      else:
          output_paths = [os.path.join(output_dir,
                                      '%s_%s.tfrecord' % (output_file_base, name))
  ...
  if use_compression:
    options = tf.io.TFRecordOptions(compression_type="GZIP")
    writers = dict((name, tf.python_io.TFRecordWriter(path, options=options))
