...
def sharded_tfrecord_reader(fname):
    ...
    def _gzip_file_reader(f):
        return tf.io.tf_record_iterator(
            f, options=tf.io.TFRecordOptions(compression_type='GZIP'))

    def _uncompressed_file_reader(f):
        return tf.io.tf_record_iterator(f)
    
    for sfname in generate_sharded_filenames(fname):
        # Choose the appropriate file reader based on file extension
        reader_fn = _gzip_file_reader if sfname.endswith('.gz') else _uncompressed_file_reader
        for record in reader_fn(sfname):
            yield record
          ...
          input_dataset = filenames.apply(
              tf.data.experimental.parallel_interleave(
                  lambda filename: tf.data.TFRecordDataset(
                      filename, 
                      compression_type='GZIP' if filename.endswith('.tfrecord.gz') else None
                  ),
                  sloppy=True,
                  cycle_length=8))
      else:
          # Use a list comprehension to create datasets based on the filename extension.
          input_dataset = tf.data.TFRecordDataset(
              [filename for filename in filenames], 
              compression_type='GZIP' if any([filename.endswith('.tfrecord.gz') for filename in filenames]) else None
          )
  else:
