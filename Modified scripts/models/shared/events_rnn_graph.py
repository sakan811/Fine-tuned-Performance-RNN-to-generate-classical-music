...
# Adding Rhythm and Harmony Loss.
...
def get_build_graph_fn(mode, config, sequence_example_file_paths=None, clr_params = {}):
  ...
  def build():
    ...
    if mode in ('train', 'eval'):
      ...
      def rhythm_consistency_loss(labels, predictions): 
          # # Calculate the difference in rhythm between consecutive labels and predictions
          # rhythm_diff_labels = tf.abs(tf.cast(labels[:-1], tf.float32) - tf.cast(labels[1:], tf.float32))
          # rhythm_diff_predictions = tf.abs(tf.cast(predictions[:-1], tf.float32) - tf.cast(predictions[1:], tf.float32))

          # # Calculate the rhythm consistency loss as the mean squared error between rhythm_diff_labels and rhythm_diff_predictions
          # rhythm_loss = tf.reduce_mean(tf.squared_difference(rhythm_diff_labels, rhythm_diff_predictions))
          # return rhythm_loss
          ...
          # Define window size. 
          # 64 is equal to 4 bars of music.
          window_size = 64

          # Create a function to generate a sliding window tensor
          def sliding_window(input_tensor, window_size):
            # Expand dims for sliding window
            input_tensor = tf.expand_dims(input_tensor, axis=0)  # Add batch dimension
            input_tensor = tf.expand_dims(input_tensor, axis=-1)  # Add channel dimension
            input_tensor = tf.expand_dims(input_tensor, axis=1)  # Add a dummy dimension for the height

            # Create a sliding window
            return tf.image.extract_patches(
                images=input_tensor,
                sizes=[1, 1, window_size, 1],
                strides=[1, 1, 1, 1],
                rates=[1, 1, 1, 1],
                padding='VALID'
            )

          windowed_labels = sliding_window(rhythm_diff_labels, window_size)
          windowed_predictions = sliding_window(rhythm_diff_predictions, window_size)

          # Calculate the rhythm consistency loss
          rhythm_loss = tf.reduce_mean(tf.math.squared_difference(windowed_labels, windowed_predictions))

          return rhythm_loss

      def harmonic_tension_loss(labels, predictions):
          # Define tension scores for specific intervals
          high_tension_intervals = [1]  # Minor 2nd
          low_tension_intervals = [0, 5, 7, 12]  # Unison, Perfect 4th, Perfect 5th, Octave
          high_tension_score = 2.0
          low_tension_score = 0.5
          default_tension_score = 1.0

          # Calculate the difference in pitch between consecutive labels and predictions
          pitch_diff_predictions = tf.abs(tf.cast(predictions[:-1], tf.float32) - tf.cast(predictions[1:], tf.float32))

          # Assign tension scores based on the intervals
          high_tension_mask = tf.reduce_any([tf.equal(pitch_diff_predictions, tf.cast(interval, tf.float32)) 
                                            for interval in high_tension_intervals], axis=0)
          low_tension_mask = tf.reduce_any([tf.equal(pitch_diff_predictions, tf.cast(interval, tf.float32)) 
                                            for interval in low_tension_intervals], axis=0)
                
          # Broadcasting the scalar tension scores to the same shape as the masks
          tension_scores = tf.where(high_tension_mask, 
                                    high_tension_score * tf.ones_like(pitch_diff_predictions), 
                                    tf.where(low_tension_mask, 
                                            low_tension_score * tf.ones_like(pitch_diff_predictions), 
                                            default_tension_score * tf.ones_like(pitch_diff_predictions)))
          
          # Calculate the harmonic tension loss as the mean of the tension scores
          harmonic_tension = tf.reduce_mean(tension_scores)

          return harmonic_tension
        
      if mode == 'train':
        
        # Initialize custom losses
        rhythm_loss = rhythm_consistency_loss(labels_flat, predictions_flat)
        harmony_loss = harmonic_tension_loss(labels_flat, predictions_flat)

        rhythm_loss_weight = flags.FLAGS.rhythm_loss
        harmony_loss_weight = flags.FLAGS.harmony_loss
        
        # Set the weights for the custom losses
        if rhythm_loss_weight > 0:
          rhythm_loss_weight = rhythm_loss_weight
        else:
          rhythm_loss_weight = 0.0
          
        if harmony_loss_weight > 0:
          harmony_loss_weight = harmony_loss_weight
        else:  
          harmony_loss_weight = 0.0
          
        # L1 Regularization
        l1_scale = flags.FLAGS.l1_regular
        if l1_scale > 0:
            l1_regularizer = tf.keras.regularizers.L1(l1=l1_scale)  # Use the scale parameter from the flag
            model_weights = [v for v in tf.trainable_variables() if 'bias' not in v.name]  # Excluding bias terms from regularization
            regularization_penalty = tf.add_n([l1_regularizer(weight) for weight in model_weights])
        else:
            regularization_penalty = 0.0

        loss = tf.reduce_mean(softmax_cross_entropy)

        if flags.FLAGS.rhythm_loss:
            loss += rhythm_loss_weight * rhythm_loss

        if flags.FLAGS.harmony_loss:
            loss += harmony_loss_weight * harmony_loss
        ...
        def cyclical_learning_rate(
            ...
            step_size,  # Size of each step in the cycle
            scale_fn=lambda x: 1.  # Scaling function
        ):
            step = tf.cast(step, tf.float32)  # Convert the step to float for calculations
            cycle = tf.floor(1 + step / (2 * step_size))  # Calculate the current cycle 
                                                          # 1 cycle is equal to 2 * 'step_size'.
                                                          
            x = tf.abs(step / step_size - 2 * cycle + 1)  # Calculate 'x' used for scaling the learning rate within the cycle
                                                          # 'x' alternates between 0 and 1.
            
            # Calculate the learning rate (LR) for the current step based on 'x' and 'cycle'.
            # LR gradually increases and decreases between 'initial_learning_rate' and 'maximal_learning_rate'.
            # If 'x' is close to 1, the LR gradually moves towards the 'initial_learning_rate'.
            # If 'x' is close to 0, the LR gradually moves towards the 'maximal_learning_rate'.
            # 'scale_fn(cycle)' is used for further scaling based on the current 'cycle'.
            lr = initial_learning_rate + (maximal_learning_rate - initial_learning_rate) * tf.maximum(0., (1 - x)) * scale_fn(cycle)
        ...
        if clr_params:
            learning_rate = cyclical_learning_rate(
                global_step,
                initial_learning_rate=clr_params['lower_bound'],
                maximal_learning_rate=clr_params['upper_bound'],
                step_size=clr_params['step_size'],
                scale_fn=lambda x: 1.
            )
        else:
